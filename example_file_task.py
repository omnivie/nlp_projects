# I have used a different language model below and got the following message 'en_core_web_sm'
#
# "UserWarning: [W007] The model you're using has no word vectors loaded, so the result of the Doc.similarity method
# will be based on the tagger, parser and NER, which may not give useful similarity judgements. This may happen if you're
# using one of the small models, e.g. `en_core_web_sm`, which don't ship with word vectors and only use context-sensitive
# tensors. You can always add your own word vectors, or use one of the larger models instead if available."


# I have noticed that with the model 'en_core_web_sm' without vectors calculated similarity is lower than as with
# a larger model 'en_core_web_md'. Therefore, 'en_core_web_sm' is not be used for similarity analysis purposes.
